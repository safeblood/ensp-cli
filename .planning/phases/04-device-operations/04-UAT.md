...
| 9 | `ensp-cli diff-config <dev1> <dev2>` compares configs | Shows color-coded differences | pending | |
| 10 | `ensp-cli audit-configs` finds inconsistencies | Groups devices by type, shows similarity scores | pending | |
| 11 | `ensp-cli import-config <device> <file> --dry-run` previews | Shows commands without executing | pending | |
| 12 | `ensp-cli exec-batch <device> "cmd1" "cmd2"` executes multiple | Executes commands sequentially, maintains state | pending | |
