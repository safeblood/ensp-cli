...
| 4 | `ensp-cli console R2 --topology <file>` attempts connection | "Connected to R2 at 127.0.0.1:2000" + session start + can type commands | ✓ pass | Works after fixes |
...
2. **Interactive session - command input not working** (FIXED)
   - Connected successfully but user cannot type commands
   - **Fix 1:** 改用 sys.stdin.buffer.read() 替代 msvcrt.getch() 以支持 Windows Terminal
   - **Fix 2:** 修复 telnetlib3 的字符串/字节处理问题（telnetlib3 使用字符串而非字节）
